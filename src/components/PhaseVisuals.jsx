...
const company = { signage: "UNTERNEHMEN" };
const site = { existingPV: 2.0, existingPVLabel: "Freifläche (Bestand)" };
const buildings = { heatSources: ["Prozess", "Trockner", "Kompress."] };
const KPI = {
  analyse: {
...
/* ── Callout badge ──────────────────────────────────────────── */
function Badge({ x, y, text, sub, icon, color, align = "left", lineFrom }) {
  const T = useTheme();
  color = color ?? T.gold;
...
/* ── Phase number badge ─────────────────────────────────────── */
function PhaseBadge({ x, y, num, icon, label, color }) {
  const T = useTheme();
  color = color ?? T.gold;
  return (
    <g>
...
function FlowParticles({ pathId, color, count = 3, dur = 3, r = 2, glow = false }) {
  const T = useTheme();
  color = color ?? T.gold;
  return (
...
/* ── Info panel ─────────────────────────────────────────────── */
function InfoPanel({ x, y, w, h, title, color, children }) {
  const T = useTheme();
  color = color ?? T.gold;
  return (
    <g>
